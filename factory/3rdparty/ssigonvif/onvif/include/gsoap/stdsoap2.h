...
/* #define DEBUG */ /* Uncomment to debug sending (in file SENT.log) receiving (in file RECV.log) and internal
                       operations (in file TEST.log) */

/* #define DEBUG_STAMP */ /* Uncomment to debug sending (in file SENT.log) receiving (in file RECV.log) and time-stamped
                             operations (in file TEST.log) */
...
#define SOAP_BUFLEN                                                                                                    \
  (65536) /* buffer length for socket packets, also used by gethostbyname_r and UDP messages, so don't make this too small */
#else
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      ...
      {                                                                                                                \
        ...
        fprintf(fdebug, "%02d%02d%02d %02d:%02d:%02d.%03d|", _localTime.wYear % 100, _localTime.wMonth,                \
                _localTime.wDay, _localTime.wHour, _localTime.wMinute, _localTime.wSecond, _localTime.wMilliseconds);  \
        CMD;                                                                                                           \
        fflush(fdebug);                                                                                                \
      }                                                                                                                \
    }                                                                                                                  \
  ...
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      ...
      {                                                                                                                \
        ...
        fprintf(fdebug, "%02d%02d%02d %02d:%02d:%02d.%06d|", _tm.tm_year % 100, _tm.tm_mon + 1, _tm.tm_mday,           \
...
struct SOAP_STD_API soap
{
  ...
  short version; /* 1 = SOAP1.1 and 2 = SOAP1.2 (set automatically from namespace URI in nsmap table), 0 indicates
                    non-SOAP content */
  soap_mode mode;
  soap_mode imode;
  soap_mode omode;
  ...
  char tmpbuf[1024];     /* in/out buffer for HTTP/MIME headers, simpleType values, element and attribute tag names, and
...
SOAP_FMAC1 int SOAP_FMAC2 soap_pointer_enter(struct soap*, const void* p, const struct soap_array* a, int n, int t,
                                             struct soap_plist**);
...
SOAP_FMAC1 void* SOAP_FMAC2 soap_id_forward(struct soap*, const char* id, void* p, size_t len, int st, int tt, size_t n,
                                            unsigned int k,
                                            void (*fcopy)(struct soap*, int, int, void*, size_t, const void*, size_t));
#endif
...
SOAP_FMAC1 int SOAP_FMAC2 soap_outunsignedShort(struct soap*, const char* tag, int id, const unsigned short* p,
                                                const char*, int);
SOAP_FMAC1 int SOAP_FMAC2 soap_outunsignedInt(struct soap*, const char* tag, int id, const unsigned int* p, const char*,
                                              int);
...
SOAP_FMAC1 extern struct soap_cookie* SOAP_FMAC2 soap_set_cookie(struct soap*, const char*, const char*, const char*,
                                                                 const char*);
